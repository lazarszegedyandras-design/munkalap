...
- A felmérés kezdetén a `C:\Munkalap` nem volt Git-repository. Az első commit (`b8fbedd`) a GitHub `main` ágán van; a tárhely nyilvános. A `main` ágon GitHub ágvédelem kér pull requestet, tiltja a kényszerített feltöltést és a törlést, és az adminisztrátorra is vonatkozik. Kötelező tesztellenőrzés még nincs.
...
- A `chore/ci-checks` ágon elkészült a GitHub Actions backend- és frontend-ellenőrzés. A workflow YAML szerkezetét ellenőriztük; 33 helyi frontend teszt sikeres. Az első GitHub-futás és a webes build eredménye még szükséges, mielőtt a két ellenőrzés kötelezővé tehető.
...
1. A `chore/ci-checks` változását pull requesten futtasd, majd a sikeres `Backend tests` és `Frontend tests and build` jobot tedd kötelezővé a `main` ágvédelemben. A teljes Docker-alapú release acceptance továbbra is külön kapu.
