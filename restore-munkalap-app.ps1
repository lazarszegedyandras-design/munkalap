...
try {
    ...
    $HasBackupAgent = $null -ne $Manifest.services.PSObject.Properties['backup_agent']
    ...
    if ($hasCurrentDeployment -and ("backup-agent" -in $currentServices)) {
        $agentIds = @(Get-ComposeContainerIds -Project $Project -Service "backup-agent")
        if ($agentIds.Count -gt 0) {
            # A Python-kód változóban és egyszeres idézőjeles literálokkal kerül
            # átadásra, hogy Windows PowerShell 5.1 se távolítsa el az idézőjeleket.
            $agentStateCode = "import sqlite3; c=sqlite3.connect('/control/backup-control.sqlite'); a=c.execute('SELECT COUNT(*) FROM jobs WHERE status IN (?, ?)', ('queued', 'running')).fetchone()[0]; r=c.execute('SELECT COUNT(*) FROM restore_jobs WHERE status IN (?, ?)', ('queued', 'running')).fetchone()[0]; x=c.execute('SELECT value FROM settings WHERE key=?', ('restore.recovery_required',)).fetchone(); lock=(x is not None and str(x[0]).lower()=='true'); print(a, r, 1 if lock else 0)"
            $agentState = Invoke-Docker -Arguments @(
                "compose", "-p", $Project, "exec", "-T", "backup-agent",
                "python", "-c", $agentStateCode
            ) -ErrorMessage "Az integralt backup/restore allapot ellenorzese sikertelen." -CaptureOutput

            $parts = (($agentState.Output | Out-String).Trim() -split '\s+')
            if ($parts.Count -lt 3) {
                throw "Az integralt backup/restore allapot valasza nem ertelmezheto."
            }
            $activeBackupJobs = [int]$parts[0]
            $activeRestoreJobs = [int]$parts[1]
            $recoveryLock = [int]$parts[2]

            if ($activeBackupJobs -gt 0 -or $activeRestoreJobs -gt 0) {
                throw "Integralt backup vagy webes restore feladat fut/varakozik. A host restore csak ezek befejezese utan indithato."
            }
            if ($recoveryLock -gt 0 -and -not $ResumeInterruptedRestore) {
                throw "A backup-agent helyreallitasi zarat jelez. Ellenorizd a rendszert, majd csak tudatosan hasznald a -ResumeInterruptedRestore kapcsolot a host-szintu helyreallitashoz."
            }
            if ($recoveryLock -gt 0 -and $ResumeInterruptedRestore) {
                Write-Warning "Aktiv helyreallitasi zar mellett folytatod a host-szintu visszaallitast. Sikeres host restore utan az Admin feluleten a HELYREALLITAS_KESZ megerositessel oldd fel a zarat."
            }
        }
    }
    ...
    if ($hasCurrentDeployment -and -not $SkipSafetyBackup) {
        if ([string]::IsNullOrWhiteSpace($SafetyBackupScript)) {
            $candidates = @(
                (Join-Path $ResolvedProjectRoot "backup-munkalap-app.ps1"),
    ...
    if ($HasBackupAgent) {
        $imageRefs += [string]$Manifest.services.backup_agent.image
    }
    ...
    Write-Host "8/9 Backup-agent, backend es frontend inditasa..."
    $applicationServices = @()
    if ($HasBackupAgent) { $applicationServices += "backup-agent" }
    $applicationServices += @("backend", "frontend")
    Invoke-Docker -Arguments (@("compose", "-p", $Project, "up", "-d", "--no-build") + $applicationServices) -ErrorMessage "Az alkalmazasi szolgaltatasok inditasa sikertelen." | Out-Null
