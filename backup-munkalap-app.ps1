...
$BackupAgentWasRunning = $false
$ApplicationStopped = $false
$RuntimeTempCreated = $false
$BackupCompleted = $false
$DbContainer = $null
$BackupAgentContainer = $null
...
try {
    ...
    $BackupAgentContainer = Get-RequiredContainerId -Service "backup-agent" -IncludeStopped
    ...
    $BackupAgentInspect = Get-ContainerInspect -ContainerId $BackupAgentContainer
    ...
    $BackupAgentWasRunning = [bool]$BackupAgentInspect.State.Running
    ...
    $BackupAgentImageRef = [string]$BackupAgentInspect.Config.Image
    ...
    if ($BackupAgentWasRunning) {
        # Windows PowerShell 5.1 eltávolíthatja a natív programnak közvetlenül
        # átadott argumentumba ágyazott dupla idézőjeleket. A Python-kód ezért
        # változóban, kizárólag egyszeres idézőjeles literálokkal szerepel.
        $activeBackupCode = "import sqlite3; c=sqlite3.connect('/control/backup-control.sqlite'); a=c.execute('SELECT COUNT(*) FROM jobs WHERE status IN (?, ?)', ('queued', 'running')).fetchone()[0]; r=c.execute('SELECT COUNT(*) FROM restore_jobs WHERE status IN (?, ?)', ('queued', 'running')).fetchone()[0]; x=c.execute('SELECT value FROM settings WHERE key=?', ('restore.recovery_required',)).fetchone(); lock=(x is not None and str(x[0]).lower()=='true'); print(a+r+(1 if lock else 0))"
        $activeBackupOutput = @(docker compose exec -T backup-agent python -c $activeBackupCode 2>&1)
        Assert-DockerSuccess "Az aktiv integralt backup/restore allapot ellenorzese sikertelen."
        $activeBackupCount = [int](($activeBackupOutput | Out-String).Trim())
        if ($activeBackupCount -gt 0) {
            throw "Integralt backup/restore feladat fut, varakozik, vagy helyreallitasi zar aktiv. Ezt elobb rendezd, majd inditsd ujra a host MASTER mentest."
        }
    }

    if ($FrontendWasRunning -or $BackendWasRunning -or $BackupAgentWasRunning) {
        $servicesToStop = @()
        if ($FrontendWasRunning) { $servicesToStop += "frontend" }
        if ($BackendWasRunning) { $servicesToStop += "backend" }
        if ($BackupAgentWasRunning) { $servicesToStop += "backup-agent" }

        docker compose stop @servicesToStop
        Assert-DockerSuccess "A frontend/backend/backup-agent leallitasa sikertelen."
    ...
    Write-Host "5/7 PostgreSQL, backend, backup-agent es frontend image-ek mentese..."
    docker image save --output $ImageSavePath $DbImageRef $BackendImageRef $BackupAgentImageRef $FrontendImageRef
    ...
    if ($IncludeContainerExports) {
        ...
        foreach ($item in @(
            ...
            @{ Name = "backup-agent"; Id = $BackupAgentContainer },
    ...
    $Manifest = [ordered]@{
        ...
        consistency = "frontend/backend/backup-agent stopped; database and runtime captured while application writes and scheduled backups were disabled"
        services = [ordered]@{
            db = [ordered]@{
            ...
            backup_agent = [ordered]@{
                container_id = $BackupAgentContainer
                image = $BackupAgentImageRef
            }
            frontend = [ordered]@{
    ...
    if ($ApplicationStopped) {
        try {
            $servicesToStart = @()
            if ($BackupAgentWasRunning) { $servicesToStart += "backup-agent" }
            ...
            if ($servicesToStart.Count -gt 0) {
                ...
                if ($LASTEXITCODE -ne 0) {
                    Write-Warning "A szolgaltatasok automatikus ujrainditasa sikertelen. Futtasd: docker compose start backup-agent backend frontend"
                }
            }
        }
        catch {
